...
import { EARLY_DECISION_SCHOOLS, earlyDecisionPath } from "../src/lib/early-decision-pilot";

test.describe("early-decision table fits its column", () => {
  test.skip(({ isMobile }) => isMobile, "desktop widths only");

  for (const width of [1440, 1024]) {
    for (const schoolId of EARLY_DECISION_SCHOOLS) {
      test(`${schoolId} at ${width}px`, async ({ page }) => {
        await page.setViewportSize({ width, height: 900 });
        const response = await page.goto(earlyDecisionPath(schoolId));
