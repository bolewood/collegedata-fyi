...
export const WAITLIST_COMPLETE_ROWS = WAITLIST_ROWS.filter(
  (row) => row.complete && row.waitListAccepted != null && row.waitListAccepted > 0,
);

export const WAITLIST_REPORTED_ANOMALY_ROWS = WAITLIST_COMPLETE_ROWS.filter(
  isLikelyWaitlistReportingAnomaly,
);

export const WAITLIST_ANALYSIS_ROWS = WAITLIST_COMPLETE_ROWS.filter(
