...
- [`load_directory.py`](./load_directory.py) — PRD 015 M1. Refreshes `institution_directory` and `institution_slug_crosswalk` from the same Scorecard CSV. Before slug assignment or any database connection, the shared finder identity guard verifies every `schools.yaml` UNITID against the incoming official name/domain and fails closed on mismatches. It reserves every reviewed `retired_aliases` slug during assignment, refuses collisions with canonical `schools.yaml` claims, and prevents generated non-primary aliases from taking a retired slug. It then applies the MVP in-scope filter (active, undergraduate-serving, two-or-four-year, degree-granting), preserves verified `schools.yaml` slugs, and generates deterministic slugs for Scorecard-only rows with collision resolution `state → city → ipeds_id`. When given the immediately previous release via `--previous-csv`, it stabilizes one-release `PREDDEG 2/3/4 → 1` regressions only when current `HIGHDEG` still confirms an associate-or-higher award. Apply runs print the live scope delta, mark rows absent from the new Scorecard vintage out of scope, refresh `refreshed_at`, and rebuild `institution_cds_coverage`. Writes a refresh summary to `scratch/scorecard/directory-refresh-<year>.json`.
...
- [`build_alignment_gap_recipe.py`](./build_alignment_gap_recipe.py) — rebuilds `web/src/lib/alignment-gap-recipe-data.ts` from public PostgREST. Panel B is `scorecard_summary` × `institution_directory` × `school_browser_rows` SAT midpoints. Panel A is `school_merit_profile` CDS H2A × Scorecard endowment and `school_browser_rows` enrollment, keeping published `$0` grants. Requires `SUPABASE_URL` and `SUPABASE_ANON_KEY`. The recipe page at `/recipes/alignment-gap` reads the generated TypeScript; it does not query live at request time.
