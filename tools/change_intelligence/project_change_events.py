...
def school_year_coverage(
    ...
    selected = select_primary_rows(rows)
    schools = set(watchlist or {school_id for school_id, _year in selected})
    with_prior = {school_id for school_id in schools if (school_id, from_year) in selected}
    with_latest = {school_id for school_id in schools if (school_id, to_year) in selected}
    pairable = with_prior & with_latest
    return {
        "watchlist_size": len(schools),
        "with_prior": len(with_prior),
        "with_latest": len(with_latest),
        "pairable": len(pairable),
        "pairable_pct": (len(pairable) / len(schools)) if schools else 0,
    }


def event_markdown_table(events: list[dict[str, Any]], limit: int = 20) -> list[str]:
    lines = [
...
def family_events(events: list[dict[str, Any]], family: str) -> list[dict[str, Any]]:
    return [event for event in events if event.get("field_family") == family]


def type_events(events: list[dict[str, Any]], *event_types: str) -> list[dict[str, Any]]:
...
def write_report(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    coverage = school_year_coverage(rows or [], watchlist, from_year, to_year)
    review_candidates = [e for e in events if e["verification_status"] == "candidate"]
    major = [e for e in events if e["severity"] == "major"]
    notable = [e for e in events if e["severity"] == "notable"]
    watch = [e for e in events if e["severity"] == "watch"]
    admissions = family_events(events, "admissions_pressure")
    international = family_events(events, "international_students")
    aid = family_events(events, "aid_affordability")
    silences = type_events(events, "newly_missing")
    blockers = type_events(events, "quality_regression", "producer_changed", "format_changed")
    lines = [
        ...
        "",
        f"- Watchlist schools: {coverage['watchlist_size']}",
        f"- Schools with prior-year primary CDS rows: {coverage['with_prior']}",
        f"- Schools with latest-year primary CDS rows: {coverage['with_latest']}",
        f"- Pairable schools for this comparison: {coverage['pairable']} ({coverage['pairable_pct']:.0%})",
        "",
        "## Event summary",
        "",
        f"- Events: {len(events)}",
        ...
        f"- Watch: {len(watch)}",
        f"- Human-review candidates: {len(review_candidates)}",
        "",
        "## Biggest admissions-pressure signals",
        "",
    ]
    lines.extend(event_markdown_table(admissions))
    lines.extend([
        "",
        "## International-student signals visible in CDS",
        "",
    ])
    lines.extend(event_markdown_table(international))
    lines.extend([
        "",
        "## Aid and affordability shifts",
        "",
    ])
    lines.extend(event_markdown_table(aid))
    lines.extend([
        "",
        "## Reporting gaps and silences worth reviewing",
        "",
    ])
    lines.extend(event_markdown_table(silences))
    lines.extend([
        "",
        "## Extraction-quality blockers",
        "",
    ])
    lines.extend(event_markdown_table(blockers))
    lines.extend([
        "",
        "## Methodology and caveats",
        "",
        "- Events are generated from selected primary Common Data Set rows only.",
        "- `newly_missing` and every `major` event require human source review before public reporting.",
        "- A missing field can mean the school omitted it, changed format, or the extractor could not recover it.",
        "- CDS events describe what changed in the published source data; they do not establish why it changed.",
        "",
    ])
    path.write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    write_report(events, args.report, args.from_year, args.to_year, rows, watchlist)
    if args.apply:
