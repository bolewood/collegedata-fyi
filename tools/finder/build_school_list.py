...
overrides (cds_url_hint, notes, scrape_policy, sub_institutions).
...
def merge(ipeds_rows: list[dict], existing: dict[str, dict]) -> list[dict]:
    ...
    for row in ipeds_rows:
        ...
        if iid in existing:
            hand = existing[iid]
            for key in ["id", "name", "cds_url_hint", "scrape_policy", "notes",
                        "sub_institutions"]:
...
# sections at the top. Hand-curated entries (with cds_url_hint, notes, etc.) are
...
#   cds_url_hint:        optional last-known URL or IR page for the CDS
    ...
    with output_path.open("w") as f:
        ...
        def write_section(label: str, items: list[dict]) -> None:
            ...
            for s in items:
                ...
                if s.get("cds_url_hint"):
                    f.write(f"    cds_url_hint: {s['cds_url_hint']}\n")
