...
**2. A non-trivial minority of school CDS PDFs are unflattened fillable forms.** Harvey Mudd's 2025-26 CDS, which we spent an audit trying to parse with layout-extraction tools, turned out to have 1,026 named AcroForm fields that `pypdf.get_fields()` reads deterministically. For those schools, extraction is a 20-line script rather than a rasterize-parse-clean pipeline. Our sample is too small (N=3) to call the distribution, but Tier 2 is not theoretical and is already verified end-to-end on HMC.
