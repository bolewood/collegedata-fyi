...
As of 2026-05-09, what's built and what isn't:
...
| IPEDS federal baseline (PRD 021) | ✅ First slice shipped 2026-05-09. Official NCES/IPEDS metadata and selected CSV table ZIPs load into release/table/column/value-label/raw-row tables; curated facts project into `ipeds_facts` and serve through `ipeds_current_facts`/`school_facts_unified`; school pages render `FederalBaselineTable` for CDS-backed and directory-only schools; the monthly `ipeds-release-probe` workflow no-ops until 10 months after the latest loaded provisional release and opens issues when NCES publishes the next final/provisional release. Browse expansion and operator dashboard remain open. |
