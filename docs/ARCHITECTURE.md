...
| [`tools/extraction_worker/tier4_extractor.py`](../tools/extraction_worker/tier4_extractor.py) | Tier 4 extractor. Converts a flattened CDS PDF to markdown via Docling (baseline config: TableFormer FAST, OCR on but not forced, 1x DPI). Scored 21/21 on critical C1 fields across 3 schools in the bake-off (commit `e15a5d3`). Output is raw markdown, not yet mapped to canonical question numbers — the schema-targeting cleaner is a follow-up. |
