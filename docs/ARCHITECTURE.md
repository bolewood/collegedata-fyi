...
The project has ten logical pipelines, each running at a different cadence and responsibility boundary:
...
8. **Change intelligence pipeline** ([PRD 019](prd/019-cds-change-intelligence.md), alpha) — operator-run. Projects selected year-over-year changes from comparable primary CDS rows into `cds_field_change_events`: material numeric deltas, newly missing/reported fields, producer/format changes, quality regressions, and recovered fields. Events default to `public_visible=false`; major events and `newly_missing` candidates require human review through `cds_field_change_event_reviews` before publication or reporting.
9. **Consumer API** — on demand. PostgREST serves `cds_documents`, `cds_artifacts`, `cleaners`, `cds_manifest`, `cds_fields`, `school_browser_rows`, `school_merit_profile`, `institution_directory`, `institution_slug_crosswalk`, `institution_cds_coverage`, and public-reviewed `cds_field_change_events` at `api.collegedata.fyi/rest/v1/`. Public Storage URLs serve the archived source files. The `browser-search` Edge Function exposes the ranked latest-per-school contract for the queryable browser; the `search_institutions` SQL RPC backs the homepage autocomplete.
10. **Frontend** — on demand. A Next.js app at `collegedata.fyi` (hosted on Vercel) consumes the PostgREST API, browser-search Edge Function, and search_institutions RPC. It renders a searchable school directory (returning both CDS-backed and directory-only schools per PRD 015 M4), a queryable school browser, per-school document archives + Scorecard baselines (with a directory-only stub for Scorecard-only schools per M5), a per-school academic positioning card backed by `school_browser_rows` SAT/ACT bands plus a localStorage-only student profile (PRD 016), a per-school admission strategy card backed by `school_browser_rows` ED/EA/wait-list/factor columns (PRD 016B), a per-school merit profile card backed by `school_merit_profile` Section H plus Scorecard fields (PRD 018), a school-page `WhatChangedCard` backed only by public-reviewed PRD 019 events, a `/match` list-builder backed by `school_browser_rows` plus directory/Scorecard enrichment and stateless profile codes (PRD 017), per-year structured field viewers, and a public coverage accountability page at `/coverage` (M6, virtualized via `@tanstack/react-virtual`). The gated `/changes` route is an operator digest only when `CHANGE_INTELLIGENCE_DIGEST_ENABLED=true` and `SUPABASE_SERVICE_ROLE_KEY` are configured. The visual system is documented in [`web/DESIGN_SYSTEM.md`](../web/DESIGN_SYSTEM.md), with canonical tokens in [`web/src/app/tokens.css`](../web/src/app/tokens.css) and a live reference page at [`/design-system/`](https://collegedata.fyi/design-system/). Every agent or contributor touching UI should read the design system first. See also [`docs/prd/002-frontend.md`](prd/002-frontend.md), [`docs/prd/010-queryable-data-browser.md`](prd/010-queryable-data-browser.md), [`docs/prd/015-institution-directory-and-cds-coverage.md`](prd/015-institution-directory-and-cds-coverage.md), [`docs/prd/016-academic-positioning-card.md`](prd/016-academic-positioning-card.md), [`docs/prd/016B-admission-strategy-card.md`](prd/016B-admission-strategy-card.md), [`docs/prd/017-match-list-builder.md`](prd/017-match-list-builder.md), [`docs/prd/018-open-college-fit-data.md`](prd/018-open-college-fit-data.md), and [`docs/prd/019-cds-change-intelligence.md`](prd/019-cds-change-intelligence.md) for product framing.
