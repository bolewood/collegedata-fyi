...
| [`tools/ipeds/load_release.py`](../tools/ipeds/load_release.py) | Dry-run/apply loader. Parses Tablesdoc metadata, loads raw CSV rows, projects the MVP fact mappings, and upserts `ipeds_releases`, table/column/value-label metadata, `ipeds_raw_rows`, and curated `ipeds_facts`. Dry runs write reports under `scratch/ipeds/`; `--apply` requires service-role credentials and should run only after migrations are applied from `main`. |
...
4. After any required migrations are merged and applied from `main`, the operator re-runs `load_release.py --apply`.
