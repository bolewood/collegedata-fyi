...
| Component | Role |
|---|---|
| [`tools/extraction_worker/worker.py`](../tools/extraction_worker/worker.py) | Python worker. Polls pending rows newest-first, with optional `--min-year-start`, `--source-format`, document-id, and requeue filters for scoped drains. Downloads archived source bytes, sniffs format with ZIP/PDF/HTML internals, performs content-based PDF year detection via `detect_year_from_pdf_bytes` and writes `cds_documents.detected_year` — authoritative per [ADR 0007](decisions/0007-year-authority-moves-to-extraction.md) Stage B — then routes to the appropriate tier extractor and writes a `cds_artifacts` row with `kind=canonical`. Idempotency checks include producer, producer version, schema version, and source SHA; `--force-reextract` opts out for managed redrains. `--reconcile-pending` repairs pending rows already covered by current artifacts, and `--deadline-minutes` stops heavy drains gracefully before CI timeout. Refreshes browser projections after successful writes unless `--skip-projection-refresh` is set. Tier 3 (docx) is still a stub pending [PRD 007](prd/007-tier3-docx-extraction.md). |
