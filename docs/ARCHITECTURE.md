...
The project has twelve logical pipelines, each running at a different cadence and responsibility boundary:

1. **Schema pipeline** — once per CDS year. Extracts the canonical schema from commondataset.org's official XLSX template into a committed JSON artifact when the year has an Answer Sheet, and synthesizes 2023-24 from the official 2023-24 PDF AcroForm tags plus the 2024-25 canonical map. Also folds per-field checkbox value decoders into the same artifact.
2. **Corpus pipeline** — once per month or so. Builds the canonical school list from IPEDS data, validates every curated UNITID against a checked-in official NCES identity snapshot, and enriches it with URL hints discovered by a pattern-ladder prober.
...
5. **Extraction pipeline** — triggered by discovery. Pulls each `extraction_pending` row, downloads the archived source, detects format, routes to a tier-specific extractor, and writes a `canonical` artifact back. The worker is source-SHA aware, can reconcile rows already covered by current artifacts without re-extracting, and supports bounded drains with graceful deadline exits. A schema-aware LLM fallback ([PRD 006](prd/006-llm-fallback.md)) runs after the deterministic Tier 4 cleaner on low-coverage docs and writes a separate `cds_artifacts` row with `producer='tier4_llm_fallback'`; consumers merge the two per Mode B only when the fallback matches the selected Tier 4 base artifact (cleaner wins, fallback fills gaps).
...
7. **Institution directory + coverage pipeline** ([PRD 015](prd/015-institution-directory-and-cds-coverage.md), shipped 2026-04-29). The trust-signal layer that makes the product feel comprehensive even when CDS data is missing. `tools/scorecard/load_directory.py` populates `institution_directory` (one row per Title-IV institution from Scorecard, ~6,300 rows, ~2,900 in scope) and `institution_slug_crosswalk` (alias -> school_id). The `directory-enqueue` edge function (operator-triggered, no cron) seeds `archive_queue` with directory-sourced schools so the resolver actually attempts them, instead of inferring "no CDS" from absence. The `refresh_institution_cds_coverage()` SQL function joins directory + cds_documents + archive_queue + school_hosting_observations + an operator override layer into the materialized `institution_cds_coverage` table, computing one of ten `coverage_status_t` values per school via the precedence in `derive_coverage_status()`. The `refresh-coverage` edge function runs on hourly pg_cron, keeping the public coverage state fresh without the 15-minute full-table rebuild pressure that depleted Supabase IO budget in June 2026. Public RLS hides `out_of_scope` rows.
8. **IPEDS federal baseline pipeline** ([PRD 021](prd/021-ipeds-coverage-layer.md), first slice shipped 2026-05-09). Loads official NCES/IPEDS release metadata and selected Data Center CSV table ZIPs into `ipeds_releases`, `ipeds_tables`, `ipeds_columns`, `ipeds_value_labels`, and `ipeds_raw_rows`, then projects source-labeled public facts into `ipeds_facts`. The serving view `school_facts_unified` joins curated facts to in-scope directory rows and preserves release type, release date, source table/variable, imputation status, and CDS-definition alignment. Latest-fact reads go through `ipeds_current_facts`, a stable view backed by `ipeds_current_facts_cache`; historical analysis queries should filter `ipeds_facts` by public `ipeds_id`, `field_key`, and `data_year`. A monthly GitHub Actions release probe checks the official Access Database page after a 10-month no-op window from the latest loaded provisional release and opens operator issues when final/provisional updates appear. See [`tools/ipeds/README.md`](../tools/ipeds/README.md).
9. **FSA institutional nonpayment pipeline.** Operator-run ingest of the public Federal Student Aid school-level nonpayment workbook (listed on [Student Aid Data → Portfolio](https://studentaid.gov/data-center/student/portfolio); stable file `nonpayment-rates.xlsx`). `tools/fsa/load.py` stamps `school_id` from Scorecard `OPEID` × in-scope directory `main_campus` and writes `fsa_releases` + `fsa_nonpayment_facts`. The serving view `fsa_nonpayment_current` is the latest vintage, public-visible matched rows only. This is not the official cohort default rate and is not a fourth named homepage source. See [`docs/designs/fsa-institutional-nonpayment.md`](designs/fsa-institutional-nonpayment.md) and [`tools/fsa/README.md`](../tools/fsa/README.md).
10. **Change intelligence pipeline** ([PRD 019](prd/019-cds-change-intelligence.md), alpha) — operator-run. Projects selected year-over-year changes from comparable primary CDS rows into `cds_field_change_events`: material numeric deltas, newly missing/reported fields, producer/format changes, quality regressions, and recovered fields. Events default to `public_visible=false`; major events and `newly_missing` candidates require human review through `cds_field_change_event_reviews` before publication or reporting.
11. **Consumer API** — on demand. PostgREST serves `cds_documents`, `cds_artifacts`, `cleaners`, `cds_manifest`, `cds_fields`, `school_browser_rows`, `school_merit_profile`, `institution_directory`, `institution_slug_crosswalk`, `institution_cds_coverage`, `ipeds_facts`, `school_facts_unified`, IPEDS release metadata tables, `fsa_nonpayment_current`, and public-reviewed `cds_field_change_events` at `api.collegedata.fyi/rest/v1/`. Public Storage URLs serve the archived source files. The `browser-search` Edge Function exposes the ranked latest-per-school contract for the queryable browser; the `search_institutions` SQL RPC backs the homepage autocomplete.
12. **Frontend** — on demand. A Next.js app at `collegedata.fyi` (hosted on Vercel) consumes the PostgREST API, browser-search Edge Function, and search_institutions RPC. It renders a searchable school directory (returning both CDS-backed and directory-only schools per PRD 015 M4), a queryable school browser, per-school document archives + Scorecard baselines, source-labeled IPEDS federal baseline tables from `school_facts_unified`, a directory-only stub for non-CDS schools, a per-school academic positioning card backed by `school_browser_rows` SAT/ACT bands plus a localStorage-only student profile (PRD 016), a per-school admission strategy card backed by `school_browser_rows` ED/EA/wait-list/factor columns (PRD 016B), a per-school merit profile card backed by `school_merit_profile` Section H plus Scorecard fields (PRD 018), a school-page `WhatChangedCard` backed only by public-reviewed PRD 019 events, a `/match` list-builder backed by `school_browser_rows` plus directory/Scorecard enrichment and stateless profile codes (PRD 017), per-year structured field viewers with XLSX/CSV spreadsheet downloads (PRD 025), and a public coverage accountability page at `/coverage` (M6, virtualized via `@tanstack/react-virtual`). The gated `/changes` route is an operator digest only when `CHANGE_INTELLIGENCE_DIGEST_ENABLED=true` and `SUPABASE_SERVICE_ROLE_KEY` are configured. The visual system is documented in [`web/DESIGN_SYSTEM.md`](../web/DESIGN_SYSTEM.md), with canonical tokens in [`web/src/app/tokens.css`](../web/src/app/tokens.css) and a live reference page at [`/design-system/`](https://collegedata.fyi/design-system/). Every agent or contributor touching UI should read the design system first. See also [`docs/prd/002-frontend.md`](prd/002-frontend.md), [`docs/prd/010-queryable-data-browser.md`](prd/010-queryable-data-browser.md), [`docs/prd/015-institution-directory-and-cds-coverage.md`](prd/015-institution-directory-and-cds-coverage.md), [`docs/prd/016-academic-positioning-card.md`](prd/016-academic-positioning-card.md), [`docs/prd/016B-admission-strategy-card.md`](prd/016B-admission-strategy-card.md), [`docs/prd/017-match-list-builder.md`](prd/017-match-list-builder.md), [`docs/prd/018-open-college-fit-data.md`](prd/018-open-college-fit-data.md), [`docs/prd/019-cds-change-intelligence.md`](prd/019-cds-change-intelligence.md), and [`docs/prd/021-ipeds-coverage-layer.md`](prd/021-ipeds-coverage-layer.md) for product framing.
...
One early-stage effort sits alongside these pipelines without being one yet: **guided college discovery** ([PRD 026](prd/026-guided-college-discovery.md)) ships a versioned content layer at [`data/discovery/`](../data/discovery/README.md) — experience-card library, interest ontology, opening deck, scenario fixtures, and the `discovery_policy_v1` artifact (`data/discovery/policy/v1.json`, executed by `tools/discovery/data_spike.py` and invariant-pinned by `tools/discovery/test_policy.py`), licensed CC BY-SA 4.0 unlike the MIT code — plus offline feasibility tooling at `tools/discovery/`. Its Milestone 0 data-spike gate passed 13/13 ([`docs/plans/prd-026-data-spike-findings.md`](plans/prd-026-data-spike-findings.md)). Its runtime is soft-launched at `/discover` (noindex, unlinked from navigation): a boundary step, an accessible 24-card sort, a plain-language preference ledger, and the discovery rounds loop — interests, six-school rounds with evidence-grounded fail-closed reasons, reactions, and a research shelf — with the session (schema v2) kept browser-local in localStorage. Rounds compose entirely in the browser: a TypeScript engine executes `discovery_policy_v1` against a versioned evidence bundle (`web/public/discovery/evidence-v1.json`, 895 schools, eligibility pre-applied at build by `tools/discovery/build_evidence_bundle.py`, served immutable) and is conformance-proven against the Python reference via a committed fixture; the student's ZIP resolves to a committed ZIP3 centroid and never leaves the device. The web app consumes committed mirrors of the content artifacts under `web/src/lib/discovery/content/`, drift-checked by `content-sync.test.ts`; the schema-constrained AI reflection step arrives in a later slice.
...
```
  ...
  │    reads XLSX "Answer Sheet" tab → extracts canonical fields        │
  │                                                                     │
  │  tools/schema_builder/build_2023_24_canonical.py                    │
  │    synthesizes 2023-24 from 2024-25 canonical map + PDF tags        │
  │                                                                     │
...
| Component | Role |
|---|---|
| [`tools/finder/build_school_list.py`](../tools/finder/build_school_list.py) | Downloads the IPEDS HD CSV from NCES, filters to 4-year degree-granting US non-profit institutions, merges with existing hand-curated entries in `schools.yaml`, and validates identities before writing back. Preserves hand-curated `discovery_seed_url`, `browse_url`, `sub_institutions`, `notes`, `scrape_policy`, and durable `retired_aliases` (plus the legacy `cds_url_hint` alias during migration). |
| [`tools/finder/identity_guard.py`](../tools/finder/identity_guard.py) | Fail-closed semantic identity guard used by CI, corpus generation, the Scorecard directory loader, and recipe generation. Checks every curated UNITID against the checked-in official NCES HD snapshot; exact reviewed exceptions cannot use wildcards and must remain in use. |
...
| [`tools/finder/schools.yaml`](../tools/finder/schools.yaml) | The canonical school list. 2,434 entries (82 hand-curated active, 2,350 IPEDS candidates awaiting probe, 2 verified absent). Post-PR-5 fields: `discovery_seed_url` (the resolver's seed URL — may be a direct PDF or a landing page), optional `browse_url` (human-friendly URL surfaced by the kids worklist), and optional `retired_aliases` for old public slugs that must permanently redirect to the canonical school. |
| [`tools/finder/school_overrides.yaml`](../tools/finder/school_overrides.yaml) | Operator-supplied per-school overrides, keyed by `school_id`. Carries manual `browse_url`, year-tagged `direct_archive_urls` for Box/Drive/SharePoint-hosted schools and for Archive.org snapshots of taken-down school PDFs, and `hosting_override` blocks that supersede inferred hosting facts. Read at edge-function runtime by `_shared/schools.ts`; NOT touched by `build_school_list.py` so IPEDS regeneration doesn't clobber operator data. |
...
| [`supabase/functions/archive-process/index.ts`](../supabase/functions/archive-process/index.ts) | Queue consumer. Invoked every 30 s by pg_cron. Claims one row via `claim_archive_queue_row()` RPC, runs `archiveOneSchool`, then atomically closes the queue lease and its `archive_queue_attempts` telemetry row through `complete_archive_queue_attempt()`. Queue id, attempt number, and `claimed_at` guard every completion against stale owners. Also supports `?force_school=<id>` for operator backfill that bypasses the queue. |
| [`supabase/functions/archive-enqueue/index.ts`](../supabase/functions/archive-enqueue/index.ts) | Cooldown-aware daily seeder. Invoked at 02:00 UTC by pg_cron. Fetches `schools.yaml` from GitHub raw, derives a deterministic `run_id` from the UTC date, and skips schools whose most recent terminal row is still inside its outcome-specific cooldown. Successful schools are checked every 7 days year-round; transient failures have no cooldown and can retry the next day; stable failures back off longer. The daily run key makes those cooldowns effective across days, while the atomic enqueue RPC and one-active-row-per-school index keep same-day calls and processor outages from creating duplicate work. Operator overrides: `?force_recheck=true` bypasses cooldown; `?cooldown_days=N` applies a validated uniform window and expands the history lookback accordingly. |
...
| [`supabase/migrations/20260713215000_latest_archive_terminal_rows.sql`](../supabase/migrations/20260713215000_latest_archive_terminal_rows.sql) | Adds the corpus-bounded `latest_archive_terminal_rows()` RPC, the atomic `enqueue_archive_queue_rows()` RPC, a one-active-row-per-school invariant, and supporting indexes. The daily enqueuer reads history in sub-row-cap corpus chunks and cannot build duplicate live jobs during processor lag. |
| [`supabase/migrations/20260716010500_archive_queue_attempt_telemetry.sql`](../supabase/migrations/20260716010500_archive_queue_attempt_telemetry.sql) through [`20260716014000_archive_queue_timeout_recovery.sql`](../supabase/migrations/20260716014000_archive_queue_timeout_recovery.sql) | Adds one ledger row per queue claim with terminal state and `duration_ms`, atomically couples claim/completion state to the ledger, records expired reclaimed leases as `timed_out`, and recovers poison rows whose attempt ledger is incomplete. Service-role-only RLS keeps operational errors private. |
...
| [`supabase/functions/archive-process/index.ts`](../supabase/functions/archive-process/index.ts) (`runForceUrls`) | The ingest target. Accepts optional `source_provenance` in the POST body, validates against an allowlist, threads it into `archiveManualUrls` → `archiveOneCandidate` → `insertFreshDocument`. Missing / invalid provenance falls through to `school_direct` for school-owned hosts (manual_urls.yaml / Playwright-collector). Wayback (`web.archive.org`) URLs without an explicit tag default to `operator_manual`. |
...
| [`supabase/functions/_shared/resolve.ts`](../supabase/functions/_shared/resolve.ts) (`rewriteWaybackUrl`) | Wayback toolbar URLs (`/web/TIMESTAMP/https://...`) return an HTML wrapper, not the original bytes. Rewrites them to the `id_` flag (`/web/TIMESTAMPid_/https://...`) so the archiver fetches the original file. Applied in `extractCdsAnchors`, `findDownloadLinks`, `resolveCdsForSchool`, and `archiveManualUrls`, same as the Drive/Box rewrites. |
| [`supabase/functions/_shared/storage.ts`](../supabase/functions/_shared/storage.ts) (`isTruncatedWaybackPdf`) | Wayback's playback cap truncates some PDFs at ~1 MB. The prefix still sniffs as `%PDF` but has no `%%EOF` trailer. Refuses those bytes so a truncated Archive.org capture is not stored as canonical CDS. |

### Extraction pipeline

| Component | Role |
|---|---|
| [`tools/extraction_worker/worker.py`](../tools/extraction_worker/worker.py) | Python worker. Polls pending rows newest-first, with optional `--min-year-start`, `--source-format`, document-id, and requeue filters for scoped drains. Downloads archived source bytes, sniffs format with ZIP/PDF/HTML internals, performs content-based PDF year detection via `detect_year_from_pdf_bytes` and writes `cds_documents.detected_year` — authoritative per [ADR 0007](decisions/0007-year-authority-moves-to-extraction.md) Stage B — then routes to the appropriate tier extractor and writes a `cds_artifacts` row with `kind=canonical`. Idempotency checks include producer, producer version, schema version, and source SHA; `--force-reextract` opts out for managed redrains. `--reconcile-pending` repairs pending rows already covered by current artifacts, and `--deadline-minutes` stops heavy drains gracefully before CI timeout. Real document failures, including suffixed `*_no_tables (0 fields)` actions, make the process exit non-zero after the JSON summary is written. Refreshes browser projections after successful writes unless `--skip-projection-refresh` is set. Tier 3 (docx) is still a stub pending [PRD 007](prd/007-tier3-docx-extraction.md). |
...
| Component | Role |
|---|---|
| [`tools/scorecard/load_directory.py`](../tools/scorecard/load_directory.py) | M1 directory loader. Reads the Scorecard CSV, then runs the shared identity guard before slug assignment or any database connection. Applies the MVP in-scope filter (active, undergrad-serving, two-or-four-year, degree-granting) and upserts `institution_directory` plus `institution_slug_crosswalk`. Preserves verified `schools.yaml` slugs and durable `retired_aliases` (`source = redirect`); deterministic slug generation with collision resolution `state → city → ipeds_id` applies only to Scorecard-only rows. Refresh summary written to `scratch/scorecard/`. |
...
| [`supabase/migrations/20260429144127_refresh_coverage_cron.sql`](../supabase/migrations/20260429144127_refresh_coverage_cron.sql) + [`20260612162500_relax_refresh_coverage_cron.sql`](../supabase/migrations/20260612162500_relax_refresh_coverage_cron.sql) | M3 cron, relaxed June 2026. The original 15-minute job is replaced by hourly `refresh-coverage-hourly` via the existing archive_pipeline vault secrets. |
| [`supabase/functions/refresh-coverage/`](../supabase/functions/refresh-coverage/) | M3 edge function. Calls the `refresh_institution_cds_coverage` RPC; returns `rows_written` and `refresh_duration_ms`. Operators can pass `{"include_histogram": true}` for a coverage-status histogram, but cron omits it to avoid a second full-table read. |
...
### IPEDS federal baseline

PRD 021 (first public slice shipped 2026-05-09) adds an official NCES/IPEDS
fact layer for schools with and without archived CDS documents. CDS remains the
school-authored source; IPEDS is a federal baseline with visible provenance.

| Component | Role |
|---|---|
| [`tools/ipeds/download_release.py`](../tools/ipeds/download_release.py) | Operator downloader. Reads the official NCES Access Database page, downloads the Tablesdoc workbook plus mapped table ZIPs, and writes a `release.json` manifest with collection year, release type, release date text, source URLs, and normalized release-date precision. |
| [`tools/ipeds/load_release.py`](../tools/ipeds/load_release.py) | Dry-run/apply loader. Parses Tablesdoc metadata, loads raw CSV rows, projects the MVP fact mappings, and upserts `ipeds_releases`, table/column/value-label metadata, `ipeds_raw_rows`, and curated `ipeds_facts`. After `--apply`, it refreshes `ipeds_current_facts_cache` and then browser source-mode flags. Dry runs write reports under `scratch/ipeds/`; `--apply` requires service-role credentials and should run only after migrations are applied from `main`. |
| [`tools/ipeds/probe_releases.py`](../tools/ipeds/probe_releases.py) | Read-only release probe. Compares loaded `ipeds_releases` to the official Access Database page, computes the next probe due date from the latest provisional release date, and reports whether the current final or next provisional release is available. |
| [`.github/workflows/ipeds-release-probe.yml`](../.github/workflows/ipeds-release-probe.yml) | Monthly scheduled workflow. No-ops until 10 months after the latest loaded provisional Access release date, then opens GitHub issues with suggested operator download/load commands when NCES publishes a matching final or provisional release. |
| [`supabase/migrations/20260509090000_ipeds_coverage_layer.sql`](../supabase/migrations/20260509090000_ipeds_coverage_layer.sql) | Core schema. Adds release metadata, table metadata, value labels, raw rows, `ipeds_facts`, `ipeds_current_facts`, and the first `school_facts_unified` serving view. Public products should use curated facts and serving views, not raw JSONB rows. |
| [`supabase/migrations/20260509120000_fix_ipeds_browser_source_modes.sql`](../supabase/migrations/20260509120000_fix_ipeds_browser_source_modes.sql) | Serving-view polish. Keeps integer display values from rendering with decimal suffixes and preserves source-mode semantics for browser/school-page usage. |
| [`supabase/migrations/20260509130000_backfill_ipeds_release_date.sql`](../supabase/migrations/20260509130000_backfill_ipeds_release_date.sql) | Backfills the official month-level release provenance for the `2024-25 provisional` load: `March 2026`, normalized to `2026-03-01` with `month` precision and a `2027-01-01` probe due date. |
| [`supabase/migrations/20260602143000_materialize_ipeds_current_facts.sql`](../supabase/migrations/20260602143000_materialize_ipeds_current_facts.sql) | Historical-scale serving follow-up. Adds the `(field_key, data_year, ipeds_id)` index for time-series analysis, backs `ipeds_current_facts` with `ipeds_current_facts_cache`, and adds the post-load cache-refresh RPC. |
| [`web/src/components/FederalBaselineTable.tsx`](../web/src/components/FederalBaselineTable.tsx) | School-page federal facts table. Groups `school_facts_unified` rows into accessible native tables, shows the release label prominently, and keeps status/definition notes available under the source column on hover/focus. |

### Consumer API

| Component | Role |
|---|---|
| Friendly JSON API | Next.js route handlers under `www.collegedata.fyi/api/*` expose no-auth, task-oriented endpoints for school search, source-labeled facts, source ledgers, sparse comparisons, field definitions, snapshot manifests, and a Streamable HTTP MCP endpoint at `/api/mcp`. These routes reuse the same public serving views as the frontend and keep provenance next to every fact for MCP/CLI/agent use. OpenAPI is published at `/openapi.json`; a lightweight guide is published at `/llms.txt`. |
| Supabase PostgREST | Exposes `public.cds_documents`, `public.cds_artifacts`, `public.cleaners`, the `public.cds_manifest` view, `public.cds_fields`, `public.school_browser_rows`, `public.school_merit_profile`, `public.institution_directory`, `public.institution_slug_crosswalk`, `public.institution_cds_coverage` (PRD 015 M3), `public.ipeds_facts`, `public.school_facts_unified`, IPEDS release metadata tables, and public-reviewed `public.cds_field_change_events` at `api.collegedata.fyi/rest/v1/`. Public-read surfaces use RLS policies; advisor-flagged serving views are `security_invoker` so callers do not bypass underlying table policies. `api.collegedata.fyi` is not an MCP host. |
| `packages/cli` and `packages/mcp-server` | Thin wrappers over the friendly JSON API. Stdio MCP is for local development and Claude Code (`COLLEGEDATA_API_BASE` may point at localhost). Hosted MCP is `https://www.collegedata.fyi/api/mcp`. They do not carry service-role credentials or duplicate source-priority logic. |
| Supabase Storage | Serves archived source files from the `sources` bucket at `{project-ref}.supabase.co/storage/v1/object/public/sources/{school_id}/{cds_year}/{sha256}.{ext}`. The `{cds_year}` segment is the archive-time resolver guess, frozen at upload time; the authoritative content-derived year lives in `cds_documents.detected_year` and is exposed as `canonical_year` in the manifest (see ADR 0007 Stage B trade-offs). SHA-addressed so every version is preserved forever (ADR 0006). Consumers discover the exact path via `cds_manifest.source_storage_path`, never by construction. Public bucket uploads allow PDF/XLSX/DOCX plus HTML snapshots served as `text/plain`; HTML login walls, bot challenges, and CDS-looking landing pages without table markup are rejected or quarantined rather than presented as source documents. |
...
| [`supabase/migrations/20260505180000_change_intelligence.sql`](../supabase/migrations/20260505180000_change_intelligence.sql) | Adds the PRD 019 `cds_field_observations` view, generated `cds_field_change_events` table, and `cds_field_change_event_reviews` table. Generated events are service-role/operator data by default. |
| [`supabase/migrations/20260505183000_public_change_events.sql`](../supabase/migrations/20260505183000_public_change_events.sql) | Adds the public-read RLS policy for only `public_visible=true` events with `verification_status in ('not_required', 'confirmed')`. This is the gate used by the school-page `WhatChangedCard`. |
...
| Component | Role |
|---|---|
| [`data/watchlists/change_intelligence_calibration.yaml`](../data/watchlists/change_intelligence_calibration.yaml) | Thirty-school calibration subset spanning selective privates/LACs, flagship publics, international-heavy schools, and extraction-edge cases. |
| [`data/watchlists/top_200_change_intelligence.yaml`](../data/watchlists/top_200_change_intelligence.yaml) | Operator-curated Top 200 seed. It is not a public ranking source and should not be exposed as product copy before the first report ships. |
| [`tools/change_intelligence/rules.yaml`](../tools/change_intelligence/rules.yaml) | Field list and thresholds for the first deterministic projector. Rates use percentage-point deltas; count/score fields use field-specific absolute/relative rules and baseline selectivity bands. |
| [`tools/change_intelligence/project_change_events.py`](../tools/change_intelligence/project_change_events.py) | Operator projector. Reads comparable primary `school_browser_rows`, applies `rules.yaml`, writes CSV/Markdown/summary artifacts, and persists `cds_field_change_events` when `--apply` is supplied. |
| [`tools/change_intelligence/review_change_event.py`](../tools/change_intelligence/review_change_event.py) | Human verification helper. Upserts a `cds_field_change_event_reviews` row, updates `verification_status`, and only allows `--publish` for `confirmed` events. |
| [`tools/change_intelligence/README.md`](../tools/change_intelligence/README.md) | Operator runbook for dry-runs, persistence, annual report seed generation, review, and publishing. |
| [`docs/plans/prd-019-spike-and-qa.md`](plans/prd-019-spike-and-qa.md) | Spike and QA summary: why PRD 019 moved forward, first calibration-run numbers, and launch gates that remain. |

### Frontend

| Component | Role |
|---|---|
| [`web/`](../web/) | Next.js 16 app hosted on Vercel at `collegedata.fyi`. Consumes the PostgREST API via `@supabase/supabase-js` with the anon key. Public pages are mostly read-only; the source-submission CTA can POST to Formspree when `NEXT_PUBLIC_FORMSPREE_ENDPOINT` is configured. No auth. |
...
| [`web/src/app/changes/page.tsx`](../web/src/app/changes/page.tsx) | PRD 019 operator digest. Disabled unless `CHANGE_INTELLIGENCE_DIGEST_ENABLED=true`; uses a server-only service-role client and exports `noindex` metadata. |
...
| [`web/src/app/schools/[school_id]/page.tsx`](../web/src/app/schools/[school_id]/page.tsx) | School detail. Resolves reviewed retired aliases from the checked-in `school-redirects.json` manifest before consulting `institution_slug_crosswalk`, then issues a permanent redirect to the canonical school. CDS-backed schools render product cards (academic positioning, admission strategy, merit profile, public-reviewed change intelligence), Scorecard outcomes, IPEDS baseline facts when present, and a document ledger that shows the three most recent CDS files before collapsing older archives. PRD 015 M4/M5: when `fetchSchoolDocuments` returns empty, falls through to `fetchInstitutionCoverage` and renders `DirectoryOnlySchoolPage` (coverage badge + summary + compact Formspree source CTA when configured + `FederalBaselineTable` + Scorecard baseline). 404 only when the resolved slug is absent from both queries. |
| [`web/src/app/schools/[school_id]/[year]/page.tsx`](../web/src/app/schools/[school_id]/[year]/page.tsx) | Year detail (SEO answer page): permanently redirects retired aliases to the canonical school-year, then renders the key stats block + full structured field viewer grouped by CDS section, plus XLSX/CSV spreadsheet download links (PRD 025) when the first document variant has structured values. |
| [`web/src/app/schools/[school_id]/[year]/cds.xlsx/route.ts`](../web/src/app/schools/[school_id]/[year]/cds.xlsx/route.ts) | PRD 025 spreadsheet routes (`cds.xlsx` here, sibling `cds.csv`). Retired aliases receive a `308` redirect to the canonical download URL. The canonical routes serve the school-year's merged extract as a downloadable workbook (README sheet + one sheet per CDS section that has extracted values) or flat CSV. `lib/spreadsheet-source.ts` assembles input from the same queries the page renders with; `lib/spreadsheet.ts` builds the rows; `lib/xlsx.ts` is a dependency-free XLSX writer. 404 when no structured values exist; CSV cells are neutralized against formula injection. |
| [`web/src/app/coverage/page.tsx`](../web/src/app/coverage/page.tsx) | PRD 015 M6 public accountability page. Server-fetches all in-scope `institution_cds_coverage` rows; `CoverageDashboard` (client) handles the histogram banner, status/state/enrollment/recency filters, and a virtualized sortable table via `@tanstack/react-virtual`. URL search params persist filter state so views are shareable. ISR can refresh more often than the hourly coverage table update, but underlying coverage data only changes after the cron or an operator-triggered refresh. |
...
| [`web/src/components/SubmissionForm.tsx`](../web/src/components/SubmissionForm.tsx) | PRD 015 M5 client form. Reads `NEXT_PUBLIC_FORMSPREE_ENDPOINT` and renders a Formspree-backed POST form when configured. The compact no-CDS CTA opens the same structured form inline; there is no `mailto:` fallback. Hidden fields carry `school_id`/`school_name`/`coverage_status` so operators can triage submissions by reason without follow-up. Rendered on directory-only school pages where `can_submit_source = true`. |
| [`web/src/components/FederalBaselineTable.tsx`](../web/src/components/FederalBaselineTable.tsx) | PRD 021 table for `school_facts_unified` rows. Uses native table semantics, visible source table/variable labels, prominent IPEDS release labels, and hover/focus details for status and CDS-definition alignment. |
...
| [`web/src/components/WhatChangedCard.tsx`](../web/src/components/WhatChangedCard.tsx) | PRD 019 school-page card. Renders only reviewed/public `cds_field_change_events`, shows labels, years, values, severity, and source links. Hides entirely when no public events exist. |
...
| [`web/src/components/SpreadsheetDownloadLinks.tsx`](../web/src/components/SpreadsheetDownloadLinks.tsx) | PRD 025 "Download spreadsheet" (XLSX) + CSV links rendered next to the source-document link on the year page; fires a `spreadsheet_downloaded` analytics event. |
| [`web/src/lib/queries.ts`](../web/src/lib/queries.ts) | Supabase query layer. `fetchExtract(documentId)` loads the selected canonical artifact and latest `tier4_llm_fallback` artifact in parallel, then merges per Mode B only if the fallback matches the selected base artifact by `base_artifact_id` or legacy markdown hash + cleaner version. The fallback fills question numbers the cleaner left blank; cleaner wins on collision. PRD 015-021 additions include institution coverage, coverage rows, academic profile rows, admission strategy rows, match-list rows, merit-profile rows, IPEDS federal baseline facts, and public-reviewed change events. |
| [`web/src/lib/change-intelligence-admin.ts`](../web/src/lib/change-intelligence-admin.ts) | Server-only service-role query helper for the gated `/changes` digest. Do not import from client components. |
...
**When NCES publishes a new IPEDS Access release:**
1. The monthly `ipeds-release-probe` workflow detects the release after the configured due date and opens a GitHub issue with operator commands.
2. An operator runs `tools/ipeds/download_release.py` to cache the official Tablesdoc workbook, selected CSV ZIPs, and `release.json` manifest under `scratch/ipeds/`.
3. The operator runs `tools/ipeds/load_release.py` without `--apply` and reviews the report for row counts, missing tables, schema drift, and projected fact counts.
4. After any required migrations are merged and applied from `main`, the operator re-runs `load_release.py --apply`; the loader refreshes `ipeds_current_facts_cache` and browser source-mode flags after fact upserts.
5. Public school pages and API consumers read curated source-labeled facts from `school_facts_unified`; raw IPEDS rows remain available for audit but are not the default product contract.
...
| Ops headless archive | GitHub Actions | Daily Chromium ingest of WAF/JS landings via `.github/workflows/ops-headless-archive.yml` (`tools/finder/headless_archive.py`); does not use Deno `force_urls` |
| PR CI workflow | GitHub Actions | Python unit tests (unittest suites + `tools/discovery` pytest), Supabase Deno tests, web unit tests (vitest), and Next.js typecheck/build via `.github/workflows/ci.yml`; no corpus drains |
...
The offline tools (schema + corpus) produce committed artifacts that ship with the repo. The online/data-load pipelines (discovery, extraction, IPEDS, coverage refresh, and consumer API) run against Supabase. The frontend runs on Vercel and reads from the Supabase API via the anon key.

---

## Where the architecture is still incomplete

As of 2026-07-12, what's built and what isn't:

| Component | Status |
|---|---|
| Schema pipeline | ✅ Built end-to-end for current canonical years. 2025-26 has 1,105 fields from the Answer Sheet, 2024-25 has its reduced Answer Sheet plus synthesized PDF tags, and 2023-24 now has a synthesized canonical schema with 1,053 fields built from the official 2023-24 PDF AcroForm tags plus the 2024-25 canonical map. Per-section-tab structural schemas for 2019-20 through 2025-26 and cross-year diffs remain committed for historical analysis. |
| Corpus pipeline | ✅ Built. `schools.yaml` tracks 2,424 schools and the Scorecard-backed institution directory tracks 6,322 institutions. The August 6, 2026 integrity audit counted 4,071 CDS documents in production. Playwright-assisted probing and `promote_landing_hints.py` tool (`b56ef97`) improved landing-page hint quality for 67 schools (`ce1a9ac`). |
...
| Extraction worker (polling loop) | ✅ Full-corpus drain completed 2026-04-20 and ongoing bounded drains now prioritize fresh CDS rows first. The worker routes by byte-derived `source_format`, strips malformed PDF null bytes, writes content-derived PDF year detection ([ADR 0007](decisions/0007-year-authority-moves-to-extraction.md)), refreshes browser projections after successful writes, and supports scoped fresh-year/document drains. June 2026 hardening added source-SHA-aware idempotency, `--reconcile-pending` for rows stranded by cancelled runs, `--deadline-minutes` for graceful CI exits, `--requeue-document-ids`, and `--force-reextract` for managed redrains. |
...
| Queryable browser backend (PRD 010+) | ✅ Backend MVP shipped 2026-04-26 and now underpins PRD 016/016B/017. `cds_fields` projects selected extraction values; `school_browser_rows` serves curated one-row-per-school-year metrics including admissions, SAT/ACT, ED/EA, wait-list, C7 factor, and app-fee columns; `browser-search` implements latest-per-school ranked search with answerability metadata. The August 6, 2026 integrity audit counted 262,537 `cds_fields` rows and 558 `school_browser_rows`. |
| Institution directory + coverage transparency (PRD 015) | ✅ M1-M6 shipped 2026-04-29. `institution_directory` (~6,300 rows, ~2,900 in scope), `institution_slug_crosswalk`, `institution_cds_coverage` materialized table refreshed hourly by pg_cron, `coverage_status_t` enum + `derive_coverage_status()` precedence helper, `directory-enqueue` operator-triggered seeder for directory-only schools, `search_institutions` RPC backing the homepage autocomplete (returns directory-only schools with coverage badges, not just CDS-backed ones), directory-only school detail page with compact Formspree source CTA when configured, public `/coverage` accountability page (virtualized table + status filters). M7 (first-party submission backend) is on the trigger-on-volume backlog. |
| IPEDS federal baseline (PRD 021) | ✅ First slice shipped 2026-05-09 and historical backfill scaled in June 2026. Official NCES/IPEDS metadata and selected CSV table ZIPs load into release/table/column/value-label/raw-row tables; older releases can fall back to official Access exports when the CSV endpoint no longer serves mapped tables. Curated facts project into `ipeds_facts` and serve through `ipeds_current_facts`/`school_facts_unified`; `ipeds_current_facts` is backed by a materialized cache for current-fact reads, while historical analysis should filter `ipeds_facts` by `ipeds_id`, `field_key`, and `data_year`. School pages render `FederalBaselineTable` for CDS-backed and directory-only schools; the monthly release probe still opens operator issues for new final/provisional releases. Browse expansion and operator dashboard remain open. |
...
| Change intelligence alpha (PRD 019) | 🟡 Alpha substrate shipped 2026-05-05. Migrations add `cds_field_observations`, `cds_field_change_events`, and `cds_field_change_event_reviews`; `project_change_events.py` projects deterministic year-over-year events from comparable primary `school_browser_rows`; `review_change_event.py` records human verdicts and publishes confirmed events; the school-page `WhatChangedCard` is public-read only for reviewed/public rows; `/changes` is an operator-only noindex digest behind env gating. First calibration run: 30 schools, 4 pairable latest/prior schools, 36 events, 2 major, 11 notable, 4 human-review candidates. Public editorial/report launch is not complete. |
...
| Frontend | ✅ Live at `collegedata.fyi` (Vercel). Includes landing with search, match list builder, queryable browser, school directory, school detail, year detail with field viewer and XLSX/CSV spreadsheet downloads, about, recipes, API docs, public-reviewed school-page change cards, an operator-only `/changes` digest, and a soft-launched guided discovery funnel at `/discover` (PRD 026; noindex, unlinked, browser-local session; card sort plus browser-composed six-school rounds with reactions and a research shelf). Consumes PostgREST plus `browser-search`. See [`docs/prd/002-frontend.md`](prd/002-frontend.md), [`docs/prd/010-queryable-data-browser.md`](prd/010-queryable-data-browser.md), [`docs/prd/017-match-list-builder.md`](prd/017-match-list-builder.md), [`docs/prd/019-cds-change-intelligence.md`](prd/019-cds-change-intelligence.md), and [`docs/prd/025-school-spreadsheet-download.md`](prd/025-school-spreadsheet-download.md). |
