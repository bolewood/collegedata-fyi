...
| Headless archive | `tools/finder/headless_archive.py`, `.github/workflows/ops-headless-archive.yml` | Same as archive process (`source_provenance` stays `school_direct`; producer `headless_download`) | Daily at `08:00 UTC` on GitHub-hosted Chromium, before the 09:17 extraction drain | Playwright crawls WAF/JS landings, discovers new CDS years from the live listing, and uploads with the browser network stack. YAML urls in `waf_blocked_urls.yaml` are fallback when the listing crawl finds nothing. Deno `force_urls` cannot fetch these hosts. Cloudflare JS/cookie challenges are in scope. AWS WAF visual captcha (NYU DAM) is detected as `waf_captcha` and skipped until the host stops issuing a puzzle to the runner IP. Tableau/PowerBI and SSO still need a person. |
