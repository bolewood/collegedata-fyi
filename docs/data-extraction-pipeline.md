...
|---|---|
| Schools publish on Box, Drive, SharePoint, Tableau, PowerBI, or WAF-protected hosts | HTML WAF/JS listings are crawled daily by `ops-headless-archive.yml`. Drive/Box URLs already in `waf_blocked_urls.yaml` are fetched the same way. Tableau/PowerBI exports, SSO login walls, and unlisted Drive files still need a hint URL or an operator-exported upload. |
