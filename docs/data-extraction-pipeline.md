...
flowchart TD
    ...
    D --> G2[Headless archive GHA<br/>Playwright WAF/JS]
    F --> G2
...
| Corpus and hints | `tools/finder/build_school_list.py`, `tools/finder/probe_urls.py`, `tools/finder/school_overrides.yaml` | `tools/finder/schools.yaml` and override hints | Monthly/ad-hoc operator work | Search misses Box/Drive-hosted files, thinly indexed subdomains, JS-only interfaces, landing pages with no searchable CDS text, and files the school took down that only survive on Archive.org. Manual hints should cover current year, prior year, and future finder behavior. Pin complete Wayback `id_` snapshots in `direct_archive_urls` when the live PDFs are gone (toolbar URLs return HTML; some captures truncate at 1 MB). |
...
| Archive process | `supabase/functions/archive-process`, shared resolver code in `supabase/functions/_shared/` | `cds_documents`, source `cds_artifacts`, Supabase Storage `sources` bucket, `school_hosting_observations`, `archive_queue_attempts` | Every `30 seconds` by pg_cron; one school per invocation | Edge runtime limits are why work is one school at a time. Each claim gets an attempt-ledger row; queue completion and attempt duration/outcome commit atomically, while reclaimed expired leases are recorded as `timed_out`. Also handles SHA-addressed source storage, SSRF guard, 50 MB file cap, 30 s fetch timeout, cooldown outcomes, sectionized CDS packages, HTML auth-wall/bot-challenge rejection, Wayback toolbar → `id_` rewrites, and truncated Wayback PDF rejection (missing `%%EOF`). HTTP 405 is a permanent `bot_challenge`; those schools are handed to the headless archive worker rather than retried on Deno fetch. |
| Headless archive | `tools/finder/headless_archive.py`, `.github/workflows/ops-headless-archive.yml`, private `bolewood/collegedata-ops` | Same as archive process (`source_provenance` stays `school_direct`; producer `headless_download`) | Hosted Chromium daily at `08:00 UTC` (scores the operator priority-100 cohort, then fetches JS/CF/Drive gaps). Residential NYU fetch at `10:00 UTC` on spoke-ops. Extraction drain is 09:17. | Playwright crawls WAF/JS landings. YAML urls in `waf_blocked_urls.yaml` are fallback. AWS WAF visual captcha (NYU DAM) is `waf_captcha` on hosted Chromium and is fetched from the private ops repo on a sticky allowlist (currently `nyu` only, cap 5). Cloudflare/Drive/Fordham stay hosted. No `top100_coverage` heartbeat in v1 — coverage JSON is an artifact. Tableau/SSO still need a person. |
| Manual upload / force archive | `tools/upload/`, `archive-process?force_urls`, `archive-upload` | Same as archive process, usually `source_provenance='operator_manual'` or explicit mirror provenance | Ad-hoc | Use when there is no crawlable landing (Tableau/PowerBI export, SSO login wall, a file already on disk, or a complete Archive.org `id_` snapshot after the school took the live PDFs down). Wayback `force_urls` without an explicit provenance tag default to `operator_manual`. WAF-gated public listings belong on the headless archive worker, not this path. New source bytes flip the document back to `extraction_pending`. |
...
| Tier 1 XLSX | `tools/tier1_extractor/extract.py` | Canonical artifact with `producer='tier1_xlsx'` | Triggered by extraction worker | Deterministic for standard templates and embedded question/answer columns. 2024-25 and 2025-26 have template cell maps; 2023-24 has a canonical schema but a visual-only XLSX template (no hidden lookup map). For those years the worker still runs Tier 1 with an empty map so the workbook-native Question Number / Answer column scanner can recover fields. Custom layouts without those columns may still end as `tier1_low_fields`. |
...
- **Finder/source fixes should cover time.** When a source discovery issue is
  ...
  add a durable hint or override so the next year is findable. If the school
  took the live PDFs down, pin complete Archive.org `id_` snapshots in
  `school_overrides.yaml` (toolbar URLs return HTML; some captures truncate
  at 1 MB without `%%EOF`).
...
|---|---|
| Schools publish on Box, Drive, SharePoint, Tableau, PowerBI, or WAF-protected hosts | HTML WAF/JS listings are crawled daily by `ops-headless-archive.yml` (Playwright). Cloudflare cookie walls and Google Drive `uc?export=download` URLs in `waf_blocked_urls.yaml` go through the same worker. AWS WAF *visual captcha* (CloudFront `x-amzn-waf-action: captcha`, NYU DAM) cannot be solved in scheduled Chromium from datacenter IPs — the worker records `waf_captcha` and keeps the YAML URL for the next run. Tableau/PowerBI exports and SSO login walls still need a hint URL or an uploaded export. |
| School took live CDS PDFs down; only Archive.org still has them | Pin year-tagged `direct_archive_urls` in `school_overrides.yaml` using Wayback `id_` URLs (toolbar captures are HTML). Prefer a CDX snapshot whose `length` matches the downloaded size; refuse truncated PDFs that sniff as `%PDF` but lack `%%EOF`. `force_urls` tags those hosts `operator_manual` unless the caller sets provenance. Re-check the live IR page if they resume publishing. |
