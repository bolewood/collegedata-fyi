...
- **Nav wordmark fails WCAG AA contrast.** The header/footer Wordmark's `.fyi` suffix renders at `opacity: 0.55` over paper, measuring ~3.6:1 against the 4.5:1 AA minimum (axe flags it on every page; found while building `/discover`, whose accessibility spec scopes around it). Fix is a design call: raise the suffix to ~`--ink-3` strength or treat the wordmark as a logo image with alt text. **Effort:** ~30 min.
...
- **FieldsView Phase 2 — accessible reconstructed CDS tables.** Drafted as [PRD 020](prd/020-accessible-cds-table-view.md). Phase 1 (PR #14) shipped the textbook gutter with per-subsection KV groups; the next step is rendering high-value subsections as real CDS-shaped HTML tables with native table semantics, list-mode fallbacks, explicit missing-data language, and VPAT/ACR-ready accessibility evidence. Start with C1 and C9, then expand to C7/H2A/B1 once the accessibility test plan passes.
...
- **[RESOLVED 2026-04-17] ~~Cross-year schema diff tool~~** Shipped as `tools/schema_builder/diff.py` with 5 year-pair diffs committed to `schemas/`:
  ...
  **Follow-ups:** resolved for the C1/C7/C9 slice in PRD 020 prep: 2024-25 now has a structural schema from the local template, and 2019-20 through 2023-24 have conservative core-table canonical overlays. Broader section overlays remain future work when a product slice needs them.
